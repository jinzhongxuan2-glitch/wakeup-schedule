...
| 导出：分享口令 | ✅ | ✅（`WKUP:` + Base64） |
| 导入：备份文件 / 分享口令 | ✅ | ✅ |
| **导入：教务系统直连（中南大学）** | ✅ | ✅（CAS 登录 + 课表 HTML 解析，自动识别排版） |
...
| **全量备份 / 恢复** | ✅ | ✅（多课表一键导出） |
| **更新提示可「忽略此版本」** | — | ✅ |
| 教务系统一键导入（1800+ 高校） | ✅ | 🟡 已支持**中南大学**（统一认证 CAS + 强智 jsxsd），其他学校未适配 |
| 日历文件导出 | ✅ | ❌ |
| 鸿蒙 / iOS 版 | ✅ | ❌（仅 Android） |
...
- **core 纯逻辑层**：`ScheduleMath`（周次/单双周/冲突检测/日期归一）、`ShareCodec`、`CsvScheduleParser`、`BackupCodec`、`jw/`（教务系统登录与课表解析）—— 零 Android 依赖，全部可单元测试
- **JUnit4 单元测试 61 个**：周次边界 12 + 冲突检测 9 + 备份编解码 6 + CSV 6 + 分享口令 4 + 教务解析 13 + 密码加密 6 + 开学日推算 5，`gradlew.bat testDebugUnitTest` 一键运行
...
主界面右上角「导入」→「从教务系统导入（中南大学）」，输入学号与密码即可：

- 登录走学校统一认证（`ca.csu.edu.cn`），课表取自本科教务系统（`csujwc.its.csu.edu.cn/jsxsd/`）
- **学号密码只用于当次登录，不保存、不写日志**
- 教务系统通常需要**校园网或学校 VPN**，校外打不开属正常现象
- 若登录页需要验证码，目前会明确提示暂不支持（可先用备份/CSV 方式导入）
- 导入页内置「调试日志」，失败时复制给开发者即可定位问题

解析器覆盖的排版差异：表头列偏移、周一/周日在前的两种顺序、同一格多门课
（`-----` / `<hr>` / 多个 `div.kbcontent`）、`1-8,10-12(周)` 逗号周次、单双周、
`[1-2节]` 与 `[0102节]` 两种节次写法、GBK/UTF-8 编码。核心逻辑在
`core/jw/`（纯 JVM），有 24 个单元测试守着。
